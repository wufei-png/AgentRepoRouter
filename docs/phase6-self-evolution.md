...
1. 显示候选列表给用户
2. 询问: "你指的是哪个项目？"
3. 用户选择后，更新 `skills/router/repo_mappings.json`
4. 添加关键字映射
5. 继续执行任务
...
3. `skills/router/repo_mappings.json` - 扩展结构
...
总结
1. ACP 命令 - 当前实现正确 ✅
npx acpx@latest <agent> exec "<task>"
支持的 agent 子命令:
- codex
- claude 
- opencode
...
- gemini
- 等等...
结论: 计划中的分开调用方式 (npx @zed-industries/claude-agent-acp) 已经过时，acpx 提供了统一的接口。
